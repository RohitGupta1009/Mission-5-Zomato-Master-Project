      ...
      <>
        ...
        <div className="hidden lg:flex mt-12 container mx-auto px-20 gap-10 ml-12">
            ...
            <div className={classnames("flex items-center gap-3 pb-2 transition duration-700 ease-in-out",{"border-b-2 border-zomato-400":type===items.id})}>
                <div className={classnames("w-16 h-16 p-3 rounded-full bg-gray-100",{[`bg-${items.activeColor}-100`]:type===items.id})}>
                    <img src={type===items.id?items.imageActive:items.imageDefault}
                     alt={items.alt}
                     className="w-full h-full"
                     />
                </div>
                <h3 className={type===items.id? "text-xl text-zomato-400":"text-xl text-gray-500 font-semibold"}>{items.name}</h3>
            </div>
                     </Link>
                ))
             }
